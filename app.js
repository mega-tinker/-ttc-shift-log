...
function mins(v){const m=String(v||"").match(/^(\d+):([0-5]\d)$/);return m?+m[1]*60 + +m[2]:null}
...
function reclassify(records){
  const groups={};
  records.forEach(r=>{if(!r.date)return;(groups[r.date]??=[]).push(r)});
  Object.values(groups).forEach(day=>{
    const pending=day.some(r=>!r.actualFinish||r.actualOt==="Pending");
    const totalActual=day.reduce((s,r)=>s+(mins(r.actualOt)||0),0);
    day.forEach(r=>{
      if(pending){r.paidOt="Pending";r.unpaidOt="Pending"}
      else if(totalActual>=10){r.paidOt=r.actualOt==="Pending"?"Pending":r.actualOt;r.unpaidOt=r.actualOt==="Pending"?"Pending":"0:00"}
      else{r.paidOt=r.actualOt==="Pending"?"Pending":"0:00";r.unpaidOt=r.actualOt==="Pending"?"Pending":r.actualOt}
    });
  });
  return records;
}
...
function recalc(){
  ...
  if(!sf||!af){$("actualOt").value=$("paidOt").value=$("unpaidOt").value="Pending";return}
  $("actualOt").value=hm(sf===af?0:diff(sf,af));
  $("paidOt").value="Pending";$("unpaidOt").value="Pending";
}
...
function switchTab(name){
  ...
  if(name==="history")renderHistory();if(name==="summary")renderSummary();
}
document.querySelectorAll(".tab").forEach(b=>b.onclick=()=>switchTab(b.dataset.tab));

function reset(){
  editingId=null;pendingPhotos=[];$("shiftForm").reset();
  $("date").value=today();$("camera").value="Unknown";$("incident").value="None";$("stepbackMissed").value="No";
  $("paid").value="";$("actualOt").value=$("paidOt").value=$("unpaidOt").value="Pending";
...
function filteredRecords(){
  ...
  let r=load().filter(x=>{
    ...
    if(ot==="paid"&&!(mins(x.paidOt)>0))return false;if(ot==="unpaid"&&!(mins(x.unpaidOt)>0))return false;if(ot==="none"&&!((mins(x.actualOt)||0)===0&&x.actualOt!=="Pending"))return false;if(ot==="pending"&&x.actualOt!=="Pending")return false;
...
function renderHistory(){
  const all=load(),records=filteredRecords(),list=$("historyList");$("filterCount").textContent=records.length+" of "+all.length+" shift(s)";list.innerHTML="";
...
function edit(id){
  ...
  ["date","routes","crew","run","bus","scheduledStart","scheduledFinish","actualFinish","stepbackTime","camera","incident","notes"].forEach(k=>$(k).value=r[k]||"");
  $("stepbackMissed").value=r.stepbackMissed||"No";recalc();document.querySelector("#shiftForm .primary").textContent="Update Shift";switchTab("new");window.scrollTo(0,0);
}
...
function total(records,f){return records.reduce((s,r)=>s+(mins(r[f])||0),0)}
function renderSummary(){
  if(!currentWeekStart)currentWeekStart=sundayOf(today());
  const all=load(),end=addDays(currentWeekStart,6),startISO=iso(currentWeekStart),endISO=iso(end);
  ...
  let lt8=0,gt8=0,actual=0,paidOt=0,unpaidOt=0,stepback=0;
  ...
  keys.forEach(date=>{
    const entries=days[date],platform=entries.reduce((s,r)=>s+(diff(r.scheduledStart,r.scheduledFinish)||0),0),dlt=Math.min(platform,480),dgt=Math.max(0,platform-480),pending=entries.some(r=>!r.actualFinish||r.actualOt==="Pending"),dactual=entries.reduce((s,r)=>s+(mins(r.actualOt)||0),0),dpaid=pending?null:(dactual>=10?dactual:0),dunpaid=pending?null:(dactual<10?dactual:0),dstep=entries.reduce((s,r)=>s+(mins(r.stepbackTime)||0),0);
    lt8+=dlt;gt8+=dgt;actual+=dactual;if(dpaid!=null)paidOt+=dpaid;if(dunpaid!=null)unpaidOt+=dunpaid;stepback+=dstep;
    const routes=[...new Set(entries.flatMap(r=>String(r.routes||"").split(/[\/,]+/).map(x=>x.trim()).filter(Boolean)))].join(" / "),buses=[...new Set(entries.map(r=>r.bus).filter(Boolean))].join(" / ");
    const tr=document.createElement("tr");tr.innerHTML=`<td>${date}</td><td>${routes||"—"}</td><td>${buses||"—"}</td><td>${hm(platform)}</td><td>${hm(dlt)}</td><td>${dgt?hm(dgt):"—"}</td><td>${pending?"Pending":dpaid?hm(dpaid):"—"}</td><td>${pending?"Pending":dunpaid?hm(dunpaid):"—"}</td><td>${dstep?hm(dstep):"—"}</td>`;body.appendChild(tr);
  });
  $("weekTotalPaid").textContent=hm(total(week,"paid"));$("weekPlatformLt8").textContent=hm(lt8);$("weekPlatformGt8").textContent=hm(gt8);$("weekPaidOt").textContent=hm(paidOt);$("weekStepback").textContent=hm(stepback);$("weekActualOt").textContent=hm(actual);$("weekPaidOtDetail").textContent=hm(paidOt);$("weekUnpaidOt").textContent=hm(unpaidOt);
  $("allShifts").textContent=all.length;const routes=[];all.forEach(r=>String(r.routes||"").split(/[\/,]+/).map(x=>x.trim()).filter(Boolean).forEach(x=>routes.push(x)));$("allRoutes").textContent=new Set(routes).size;$("allBuses").textContent=new Set(all.map(r=>r.bus).filter(Boolean)).size;$("allIncidents").textContent=all.filter(r=>r.incident&&r.incident!=="None").length;const photos=all.flatMap(r=>r.photos||[]);$("allPhotos").textContent=photos.length;$("allPaid").textContent=hm(total(all,"paid"));const ex=photos.filter(expired).length;$("expiredInfo").textContent=ex?ex+" photo(s) older than 6 months and not marked Keep.":"No expired photos.";
}
...
function openWork(type){
  ...
  if(type==="paid"){title="Overall Paid";const m={};all.forEach(r=>{const y=(r.date||"").slice(0,4);m[y]=(m[y]||0)+(mins(r.paid)||0)});rows=Object.entries(m).sort((a,b)=>b[0].localeCompare(a[0])).map(([k,v])=>[k,hm(v),()=>{$("filterFrom").value=k+"-01-01";$("filterTo").value=k+"-12-31";switchTab("history")}])}
  box.innerHTML='<div class="drill-title"><h3>'+title+'</h3><button id="closeDrill" class="secondary">Close</button></div>';
  rows.slice(0,100).forEach(r=>{const d=document.createElement("div");d.className="drill-row";d.innerHTML='<div><strong>'+r[0]+'</strong><div class="drill-meta">'+r[1]+'</div></div><button>View</button>';d.querySelector("button").onclick=r[2];box.appendChild(d)});$("closeDrill").onclick=()=>box.classList.add("hidden");
}
...
currentWeekStart=sundayOf(today());reset();renderHistory();renderSummary();
if("serviceWorker"in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=6f").catch(()=>{}));
});