...
function renderSummary(){
  ...
  keys.forEach(date=>{
    ...
    const platform=entries.reduce((s,r)=>s+diff(r.scheduledStart,r.scheduledFinish),0);
    const dlt=Math.min(platform,480),dgt=Math.max(0,platform-480);
    ...
    lt8+=dlt;gt8+=dgt;actual+=dactual;paidOt+=dpaid;unpaidOt+=dunpaid;stepback+=dstep;
  ...
  $("weekPlatformGt8").textContent=hm(gt8);
